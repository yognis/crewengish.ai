  ...
  const startRecording = async () => {
    ...
    try {
      if (
        ...
        setError("Tarayiciniz ses kaydini desteklemiyor.");
        return;
      }
      ...
      mediaRecorder.onstop = () => {
        ...
        const blobType =
          audioMimeTypeRef.current || audioMimeType || "audio/webm";
    ...
    <div className="space-y-4">
      ...
      {state === "review" && recordingBlob && (
        <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-6 text-center">
          <p className="text-sm font-medium text-gray-700">Kayıt tamamlandı</p>
          <div className="flex items-center justify-center gap-3 text-2xl font-mono text-gray-900">
            {formattedTime}
          </div>
          <div className="h-2 w-full rounded-full bg-gray-100">
            <div
              ...
              style={{
                width: `${Math.min((timer / maxDuration) * 100, 100)}%`,
              }}
            />
          </div>
          <button
            ...
            className="inline-flex w-full items-center justify-center rounded-lg bg-thy-red px-4 py-2 text-sm font-semibold text-white shadow hover:bg-thy-darkRed"
          >
            Kaydı Gönder
          </button>
        </div>
